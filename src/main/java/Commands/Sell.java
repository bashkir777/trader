...
public class Sell implements Commands {
    ...
    public Sell(String ticker, DatagramSocket ds, int serverPort, ArrayList<Long> idArr) throws WrongServerWork, IOException {
    ...
    public Sell(String ticker, float price, DatagramSocket ds, int serverPort, ArrayList<Long> idArr) throws WrongServerWork {
        this.ticker = ticker.toUpperCase();
        this.price = price;
        this.quantity = (((MyBalance.getMyBalance(ds, serverPort))*5)/4)/price;
        this.ds = ds;
    ...
    public Sell(String ticker, float price, float quantity, DatagramSocket ds, int serverPort, ArrayList<Long> idArr) {
        this.ticker = ticker.toUpperCase();
    ...
    @Override
    public void run() {
        SendJson.sendJson(MakeJson.makeApplicationJson("short", price, quantity, ticker), ds, serverPort);
        try{
            ds.setSoTimeout(3000);
            ServerAnswer serverAnswer = ReceiveJson.receive(ds);
            String answer = serverAnswer.getAnswer();
            if (answer.equals("exception")){
                System.out.println("Не удалось создать заявку на продажу, проверьте баланс кошелька, " +
                        "цену и количество токенов");
            }else {
                String str = answer.split("'origQty'")[1].split(",")[0];
                String str2 = answer.split("'price'")[1].split(",")[0];
                System.out.println("Заявка на продажу " + str.substring(3, str.length()-1) + " токенов " + ticker
